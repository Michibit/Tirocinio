...
def collect_required_data():
    ...
    print("\n\n", "="*80, sep="")
    print(
        " "*10, "\033[1m\033[94mThingsBoard device provisioning with basic authorization example script.\033[0m", sep="")
    print("="*80, "\n\n", sep="")
    host = input(
    ...
    port = input(
        "Please write your ThingsBoard \033[93mport\033[0m or leave it blank to use default (1883): ")
    config["port"] = int(port) if port else 1883
    config["provision_device_key"] = input(
        "Please write \033[93mprovision device key\033[0m: ")
    config["provision_device_secret"] = input(
        "Please write \033[93mprovision device secret\033[0m: ")
    device_name = input(
        "Please write \033[93mdevice name\033[0m or leave it blank to generate: ")
    if device_name:
...
class ProvisionClient(Client):
    ...
    def __on_connect(self, client, userdata, flags, rc):  # Callback for connect
        if rc == 0:
            ...
            print("[Provisioning client] Sending provisioning request %s" %
                  provision_request)
            # Publishing provisioning request topic
            client.publish(self.PROVISION_REQUEST_TOPIC, provision_request)
        else:
            print("[Provisioning client] Cannot connect to ThingsBoard!, result: %s" %
                  RESULT_CODES[rc])

    def __on_message(self, client, userdata, msg):
        decoded_payload = msg.payload.decode("UTF-8")
        print("[Provisioning client] Received data from ThingsBoard: %s" %
              decoded_payload)
    ...
    def get_new_client(self):
        ...
        if client_credentials:
            ...
            # Setting username and password for ThingsBoard client
...
# Callback for connect with received credentials
def on_tb_connected(client, userdata, flags, rc):
    if rc == 0:
        print("[ThingsBoard client] Connected to ThingsBoard with credentials: username: %s, password: %s, client id: %s" % (
            client._username.decode(), client._password.decode(), client._client_id.decode()))
    else:
        print("[ThingsBoard client] Cannot connect to ThingsBoard!, result: %s" %
              RESULT_CODES[rc])
...
if __name__ == '__main__':
    ...
    # Getting client with provisioned data
